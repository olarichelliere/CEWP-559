...
function showNewCategorie(event) {
    event.preventDefault();
    
    hideAllSections();

    var htmlContainer = document.getElementById('new_categorie_container');
    htmlContainer.style.display = "block";

    document.getElementById("new_categorie_title").value = '';
    document.getElementById("new_categorie_desc").value = '';
    document.getElementById("new_categorie_image").value = '';
}
...
function createCategorie(event){
    event.preventDefault();
    
    var title = document.getElementById("new_categorie_title").value;
    var desc = document.getElementById("new_categorie_desc").value;

    var file = document.getElementById("new_categorie_image").files[0];
    ...
    httpRequest('POST', '/categories/', data, function (newRecord) {
        console.log('Successful creation of new categorie', newRecord);

        fileUpload(`/categories/${newRecord.id}/image`, file, function(){
            console.log('File uploaded successfully!');
            document.getElementById("categorie_btn").click();
        });
...
function hideAllSections() {
    ...
    document.getElementById("new_categorie_container").style.display = "none";
}
...
function loaded() {
    ...
    document.getElementById("new_categorie_btn").addEventListener('click', showNewCategorie, false);
    document.getElementById("items_btn").click();
}